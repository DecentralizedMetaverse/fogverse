...
/// </summary>
public class ObjectLocationBasedSyncManager : MonoBehaviour
{
    Dictionary<string, GameObject> excludedObjects = new();  // 表示制限により、座標同期を停止するObujects
    P_Location selfLocationData = new();
    // Dictionary<string, object> selfAnimationData = new();
    P_Animation selfAnimationData = new();

    private void Start()
    {
        GM.Add<string, (int, int, int)>("UpdateChunk", UpdateChunk);
        GM.Add<P_Location>("SetSelfLocationData", (data) => selfLocationData = data);
        GM.Add<P_Animation>("SetSelfAnimationData", (data) => selfAnimationData = data);
    ...
    async UniTask SendLocation(int intervalDistance)
    {
        ...
        while (true)
        {
            ...
            foreach (var id in list)
            {
                ...
                selfLocationData.time = intervalTimeSec;
                var bytes = MemoryPackSerializer.Serialize(selfLocationData);
                GM.Msg("RTCSendDirect", MessageType.Location, id, bytes);
            }
    ...
    async UniTask SendAnimation(int intervalDistance)
    {
        ...
        while (true)
        {
            ...
            foreach (var id in list)
            {
                ...
                var bytes = MemoryPackSerializer.Serialize(selfAnimationData);
                GM.Msg("RTCSendDirect", MessageType.Animation, id, bytes);
            }
            await UniTask.Delay(System.TimeSpan.FromSeconds(intervalTimeSec));
        }
    }
